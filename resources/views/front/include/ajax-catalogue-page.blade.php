...
<div class="col-xxl-3 col-xl-3 col-lg-3 col-md-4 col-sm-12 col-12 each-catalog-product" data-product-type="{{ isset($product->is_public) && $product->is_public == 1 ? 'public' : 'private' }}">
    ...
    <article>
        <div class="product-images">
          ...
          if(isset($product) && $product->p_pricetype == 'dynamic')
          {
              ...
              $original_price = (isset($product->p_fix_price) ? $product->p_fix_price : 0) - $overall_discount_amount;
          }
      @endphp
      ...
      @endif
        ...
        <div class="product-details-section">
            <a href="{{ route('front.detail.products',['slug' => $product->p_slug]) }}" aria-label="Product Namr">
                <h3>{{ isset($product->p_title) ? Str::limit($product->p_title, 60) : '' }}</h3>
            </a>
            ...
            <div class="product-action-section">
                <a onclick="addToWishlist('{{$product->id}}','{{$product->p_title}}','{{$product->p_slug}}','','{{ $product->p_grand_price_total }}','simple');" aria-label="Whishlist" class="whishlist">
                ...
                @if(isset($product->p_status) && $product->p_status != null && $product->p_status == 'by_order')
                    ...
                    <a onclick="addToCart('{{$product->id}}','{{$product->p_title}}','{{$product->p_grand_price_total}}',null,'','simple');" aria-label="Shop Now" class="shop-now desktop-show-now"><span class="shop-now-span">Shop Now</span></a>
                    <a onclick="addToCart('{{$product->id}}','{{$product->p_title}}','{{$product->p_grand_price_total}}',null,'','simple');" aria-label="Shop Now" class="shop-now mobile-show-now">
                ...
                @elseif(isset($product->p_status) && $product->p_status != '' && $product->p_status != null && $product->p_status == 'ready_stock')
                    ...
                    @if((isset($product->p_avail_stock_qty) && $product->p_avail_stock_qty != 0 && $product->p_avail_stock_qty != '') || $product->p_avail_stock_qty == null)
                        <a onclick="addToCart('{{$product->id}}','{{$product->p_title}}','{{$product->p_grand_price_total}}',null,'','simple');" aria-label="Shop Now" class="shop-now desktop-show-now"><span class="shop-now-span">Shop Now</span></a>
                        <a onclick="addToCart('{{$product->id}}','{{$product->p_title}}','{{$product->p_grand_price_total}}',null,'','simple');" aria-label="Shop Now" class="shop-now mobile-show-now"><img src="{{ asset('front/src/images/p-cart.svg') }}" width="18" height="18"></a>
                    @else
