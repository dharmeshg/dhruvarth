        ...
        <div class="row align-items-center">
            <div class="col-xxl-8 col-xl-8 col-lg-8 col-md-8 col-sm-12 col-12 left-side">
                <span class="d-flex page-nav-text">
                    ...
                    <a aria-label="home icon">Collections</a>
                </span>
...
<section class="common-padding common-section catalogues-section product-box-section catalogues-page-product">
    <div class="container-md" id="all_collection">

        <div class="row" id="collection_list">
            @if(isset($collection) && count($collection) > 0)
                @foreach($collection as $item)
                    @php
                    if($item->cover_image && $item->cover_image != '' && $item->cover_image != null)
                    {
                    ...
                    @endphp
                    <div class="col-xxl-3 col-xl-3 col-lg-3 col-md-4 col-sm-12 col-12 cols-cat" data-item="{{ isset($item->name) ? $item->name : '' }}">
                        <article>
                            <div class="product-images">
                                <div class="new-collections-tagline number-text">
                                    @if(isset($item->catalogue_id) && $item->catalogue_id != '') 
                                        @php 
                                            $catalogueId = $item->catalogue_id;
                                            if (substr($catalogueId, 0, 1) == ',') {
                                                $catalogueId = substr($catalogueId, 1);
                                            }
                                            $catalogs_count = explode(",", $catalogueId); 
                                        @endphp 
                                        {{count($catalogs_count)}} CATALOGUE
                                    @else
                                        0 CATALOGUE
                                    @endif 
                                </div>
                                <a href="{{route('collection.catalogues', ['id' => $item->slug])}}" aria-label="{{ isset($item->name) ? $item->name : '' }}">
                            ...
                            <div class="product-details-section">
                                <div class="inner-section">
                                    <a href="{{route('collection.catalogues', ['id' => $item->slug])}}" aria-label="{{ isset($item->name) ? $item->name : '' }}">
                                        <h3>{{ isset($item->name) ? Str::limit($item->name, 50) : '' }}</h3>
                                    </a>

                                    <a onclick="share_model('{{$item->id}}','collection')" aria-label="Add To Card" class="add-to-cart">
    ...
    <script type="text/javascript">
        $('#search_id').click(function(){
           ...
           var inputValue = $('#search_all_prodcut').val();
            ...
            $("#collection_list").addClass("inactive");
            $(".cols-cat").hide();
            ...
            $("#collection_list").each(function(){
                $(".cols-cat").each(function(){
                    ...
                    if(item.toUpperCase().indexOf(inputValue.toUpperCase()) != -1){
                        $(this).parents("#collection_list").removeClass("inactive");
                        $(this).show();
                    }
                });
            });
        ...
        $(document).on('click', '.clear-filter', function() {
            ...
            $('.clear-filter').hide();
            $('#all_collection').load(' #collection_list');
        });
    </script>
@endsection