...
<style>
                            ...
                            <div class="pcoded-inner-content">
                                <div class="custom_breadcum">
                                    <ul>
                                        ...
                                        <li class="breadcum-item active"><a href="javascript:;"> Daily Status </a></li>
                                    </ul>
                                </div>
                                <div class="main-body">
                                            ...
                                            <div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                <div class="card Recent-Users mb-4">
                                                    <div class="card-header">
                                                        <h5>Daily Status</h5>
                                                        <a href="{{route('daliystatus.add')}}" class="add-article-btn">Add Daily Status</a>
                                                    </div>
     ...
     $(document).ready(function() {
        $("#from_date").datepicker({ 
         ...
         var table = $('#myTable').DataTable({
            ...
            ajax: {
                url: admin_url + "daily-status/list",
                type: 'post',
                data: {
                    _token: token,
                },
        ...
        if ($('#status_select').val() != '2' || $('#from_date').val() != '' || $('#to_date').val() != '') {
          
            var formData = $('#search_daily_status').serialize();
            $.ajax({
            url: '{{route('daliystatus.search')}}', 
            type: 'POST',
            data: formData,
    ...
    $(document).on("click", "#clear_filter", function() {
        ...
        $('#clear_filter').css('display','none');
         ...
         $(document).on('click', '.daliy_status_delete', function(e) {
            e.preventDefault();
            var deleteUrl = $(this).data('href');
            Swal.fire({
                title: 'Are you sure?',
                text: 'You are about to delete the Daliy Status!',
                icon: 'warning',
        ...
        $(document).on('click', '#is_featured', function() {
            ...
            var checkbox = $(this);
            $.ajax({
                url: admin_url + "check/featured-daliy-status",
                type: "post",
                data: {
                    _token: token,
                    isChecked: isChecked,
        ...
        $(document).on('click', '#pinned_chekbox', function() {
            var token = $("meta[name='csrf-token']").attr("content");
            var id = $(this).attr("data-id");
            var isChecked = $(this).is(':checked');
            $.ajax({
                url: admin_url + "check/pinned-chekbox-status",
                type: "post",
                data: {
                    _token: token,
                    isChecked: isChecked,
